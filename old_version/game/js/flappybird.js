...
var fbLoggedIn = false; // Indicates if the user is logged in via Facebook.
...
function init() {
  ...
  manifest = [
    ...
    { src: "img/submit.png", id: "highscore" },
    { src: "img/flappy-bird-ad-panel.png", id: "adpanel" },
    { src: "fonts/FB.eot" },
    { src: "fonts/FB.svg" },
    { src: "fonts/FB.ttf" },
    { src: "fonts/FB.woff" },
...
function die() {
  ...
  if (checkHighScore()) {
    newHighScoreBtn = new createjs.Bitmap(loader.getResult("highscore"));
    newHighScoreBtn.alpha = 0;
    newHighScoreBtn.x = w / 2 - newHighScoreBtn.image.width / 2;
    newHighScoreBtn.y = h / 2 - newHighScoreBtn.image.height / 2 - 250;
    stage.addChild(newHighScoreBtn);
    createjs.Tween.get(newHighScoreBtn).to(
      { alpha: 1, y: newHighScoreBtn.y + 50 },
      400,
  ...
  createjs.Tween.get(start)
    .to({ alpha: 1, y: start.y + 50 }, 400, createjs.Ease.sineIn)
    .call(addClickToStart);
  createjs.Tween.get(share).to(
    { alpha: 1, y: share.y + 50 },
    400,
    createjs.Ease.sineIn
  );
}
function removeStart() {
  $(stage).off("click").off("tap");
  stage.removeChild(start);
  stage.removeChild(share);
  stage.removeChild(newHighScoreBtn);
}

function checkHighScore() {
  return false; // TODO: Make call to PHP / check cookie to determine if high score is reached
}
function submitHighScore() {
  // Clear the stage for the high score screen
  removeStart();
  counter.alpha = counterOutline.alpha = 0;
  counterShow = false;
  //When user clicks ok, either will be prompted for FB auth, or taken to submit high score screen
  if (fbLoggedIn) {
    console.log("Logged into Facebook");
    // Submit the user's high score and display high score table
  } else {
    restart();
  }
}
function addClickToStart() {
  ...
  $(newHighScoreBtn).on("click", onButtonClicked).on("click", submitHighScore);
  $(newHighScoreBtn).on("tap", onButtonClicked).on("tap", submitHighScore);
  ...
  $(share).on("click", onButtonClicked).on("click", goShare);
  $(share).on("tap", onButtonClicked).on("tap", goShare);
  var startOnClick = function () {
    if (!buttonClicked) restart();
    buttonClicked = false;
  };
  $(stage).on("click", startOnClick).on("tap", startOnClick);
}
...
function goShare() {
  ...
  window.open(
    "https://twitter.com/share?url=http%3A%2F%2Ffreeflappybird.org&text=I scored " +
      countText +
      " on Free Flappy Bird at"
  );
}
...
function tick(event) {
  ...
  if (started && !dead) {
    if (pipeDelay == 0) {
      ...
      pipe.x = w + 600;
      pipe.y = (ground.y - 2 * gap) * Math.random() + 1.5 * gap;
      pipes.addChild(pipe);
